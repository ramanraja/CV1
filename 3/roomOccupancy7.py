# Motion Detection with hysteresis; sytem timer; capture transition frames in files
...
take_snapshots = True   # to take a snapshot every time the occupancy state changes
---------------------------------------------------------------------- 
'''
...
while True:
    ...
    if take_snapshots & state_changed:
        file_name = "captured/Capture_" +datetime.datetime.now().strftime("%d-%b-%y, %I_%M_%S_%p") +".jpg"
        print file_name
