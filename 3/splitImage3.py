# Cut off marked circular zones from an image and plot B,G,R histograms  (TODO: apply the mask within calcHist call)
...
for i in range(len(points)): 
    ...
    histb = cv2.calcHist([b], [0], None, [64], [0,256])
    histg = cv2.calcHist([g], [0], None, [64], [0,256])
    histr = cv2.calcHist([r], [0], None, [64], [0,256])
    plt.subplot(2,2,i+1)
    plt.xlim([-1, 65])
