...
pic2_name = 'parking100_4.bmp' #jpg'
print 'Number of cmd line arguments:', len(sys.argv) 
print 'Argument List:', str(sys.argv)
if(len(sys.argv) > 1):
    pic2_name = str(sys.argv[1])
print 'Input file:', str(sys.argv[1])
pic1 = misc.imread('parking100_1.bmp')  #jpg')
print type(pic1)
print pic1.dtype
...
for row in range(10):
    for col in range(10):
        if(index in free):
            ...
            draw.text((x,y),"FREE", font=font)
        index += 1

plt.figure(figsize=(pic2.shape[0], pic2.shape[1]))
plt.imshow(img)
plt.show()     
